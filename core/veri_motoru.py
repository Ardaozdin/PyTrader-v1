...
def piyasa_genel_analiz():
    # ... (Eski kod aynı kalıyor) ...
    try:
        exchange = borsa_baglantisi()
        tickers = exchange.fetch_tickers()
        items = []
        for s, d in tickers.items():
            if "/USDT" in s and "UP/" not in s and "DOWN/" not in s and d['quoteVolume'] > 10000000:
                items.append({
                    "Coin": s.replace("/USDT", ""),
                    "Sembol": s,
                    "Fiyat": float(d['last']),
...
# --- YENİ EKLENEN: TOP 50 LİSTESİ ---
@st.cache_data(ttl=300) # 5 dakikada bir listeyi yenile
def top_50_coin_getir():
    """En yüksek hacimli 50 USDT çiftini getirir"""
    try:
        exchange = borsa_baglantisi()
        tickers = exchange.fetch_tickers()
        # Hacme göre sırala ve ilk 50'yi al
        usdt_pairs = {k: v for k, v in tickers.items() if k.endswith('/USDT') and 'UP/' not in k and 'DOWN/' not in k}
